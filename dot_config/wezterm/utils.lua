...
function M.generate_session_name()
	local hostname = wezterm.hostname()

	local prefix = hostname:match("^%a%a")
	if prefix then
		prefix = prefix:upper()
	else
		prefix = "PC"
	...
	local digits = hostname:match("%d%d%d%d") or "0000"

	return "from-" .. prefix .. digits
end
